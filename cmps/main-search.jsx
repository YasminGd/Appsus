...
class _MainSearch extends React.Component {
    ...
    componentDidUpdate() {
        if (this.state.currentPage !== this.props.currentPage) this.setState({
    ...
    render() {
        const filter = this.state.currentPage === '/note' ? 'noteFilter' : 'mailFilter'

        return <section className="main-search-container">
            ...
            <input className="main-search" type="search" placeholder="Search" value={this.state[filter]} name={filter} onChange={this.handleChange} />
        </section>
    }
}
